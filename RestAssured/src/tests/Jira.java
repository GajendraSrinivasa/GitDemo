...
public class Jira {
	...
	// add a comment in the issue
		String commentBody = "Hi, it is a bug in jira";
	...
	// add an attachment to the issue using multipart()	
		...
		for(int i=0; i<commentCount; i++){
			...
			System.out.println(extractCommentId);
			if(extractCommentId.equalsIgnoreCase(commentID)){
				String extractComment = js2.get("fields.comment.comments["+i+"].body").toString();	
				System.out.println(extractComment);
				Assert.assertEquals(extractComment, commentBody);// assertion fails due to additional extra space at the end.
			}
		}
